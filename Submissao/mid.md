...
standby at the language level [4], both of which were accepted for
publication.
...
In particular, we built an RF module driver in Céu from scratch, which also
requires an SPI driver.
...
Our main quantitative result so far are the first full-application experiments
...
mode.
The "Blink" example in Céu relies timer standby, which is not the most
...
since it must be programmed explicitly and is very application dependent.
...
However, since the grant became available only on April, we could not hire such
professional.
<!--
...
The next semester I will offer, for the first time, a course on Embedded
Software which did not exist in the graduate program.
This course and our new front on the educational programming environment are
important to bring new students.
Nevertheless, in May, we got our first long-term committed student.
...
The next steps are focused on three fronts:

1. Evaluate realistic IoT applications:

From the original plan, this is the last step and probably the most laborious:
...
for a couple of months and we expect to start this course in the first semester
of the next year.

3. Work towards formal verification of program specifications:

Céu has strong safety properties that apply to all of programs as discussed in
our last paper (i.e., termination, determinism, and memory boundedness).
...
Item 1 will concentrate most of our efforts, and would complete our intended
research for the first year.
...
correctness (including energy efficiency) will also have a high impact outside
of our subfield.

# 8. Por favor, descreva como tem sido o uso dos recursos via Funarbe (em português).

A comunicação com os funcionários não é demorada, eles sempre têm boas
respostas e conhecimento sobre as dúvidas e demandas, e sempre tentam resolver
os problemas proativamente.

O meu projeto requisitou R$70000, mas descontando 6% da Funarbe e 5% da UERJ,
sobram R$62300.
Já foram gastos em torno de R$27000 divididos da seguinte forma:

```
Equipamentos:    R$  7500  (notebook, multímetro, arduinos, sensores, etc)
Congresso Int'l: R$ 15000  (passagem, seguro, inscrição, 7 diárias)
Alunos:          R$  4500  (a seguir)
```
