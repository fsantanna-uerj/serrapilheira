...
concurrently and could verify that the applications remain in deepest standby
modes for longest periods of time.
...
allows for unmmodified programs in \CEU to take advantage of standby
automatically.
%
...
starts an analog-to-digital conversion in the single channel \code{A0}.
...
%
This way, race conditions are only possible within \code{async/isr} blocks,
...
releases the \code{ADC} subsystem.

The \code{finalize} construct of \CEU executes the nested code whenever the
...
%
It is possible that the \code{AnalogRead} terminates normally or that a radio
message arrives during the A/D conversion, which causes the \code{AnalogRead}
to abort abruptly.
...
%
Figure~\ref{lst.pm} implements the \code{pm\_sleep} function for the
\emph{ATmega328p} microcontroller which the PMR calls when the program is idle.
%
...
With all the standby infrastructure set, the program in
Figure~\ref{lst.inversion}.b will automatically take advantage of the longest
periods of time and deepest sleep modes possible.
...
encompass a power management runtime and support for interrupt service routines
in the language.
%
...
standby.
%
We then take energy-unaware programs in \CEU and show that, using the proposed
infrastructure, they can take advantage of the longest periods of time and
deepest sleep modes possible.
...
rewritten in \CEU take advantage of transparent standby modes.
%
