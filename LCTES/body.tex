...
variety of peripherals, such as GPIO, ADC, USART, SPI, and the nRF24L01
transceiver.
...
that executes forever in a loop a sequence of operations as follows:
    ...
    performs an analog to digital conversion (ln. 3--4), and
...
Another advantage of the example in Figure~\ref{lst.inversion}.b is that the
structure of the program indicates which peripherals are active at a given
time.
%
For instance, when the program is awaiting concurrently in lines 2 and 7,
only the radio and ADC can awake the program.
Hence, the language runtime can choose the most energy-efficient sleep mode
that allows these peripherals to awake the microcontroller from appropriate
interrupts.
%
Since the semantics of \CEU forces the program to always reach \code{await}
statements in all active lines of execution, it is always possible to put the
microcontroller into the optimal sleep mode.
...
\end{lstlisting}
%\rule{8.4cm}{0.37pt}
\caption{ \CEU driver for the analog-to-digital converter.
\label{lst.adc}
}
\end{figure}
