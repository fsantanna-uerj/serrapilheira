...
Figure~\ref{lst.adc} shows the driver for .
...
async/isr ADC_vect_num do
  var int value = <...>; // register with the value read
...
\begin{figure}[t]
\begin{lstlisting}[numbers=left]
#define PM_GET(peripheral) \
        bitRead(pm, peripheral)
#define PM_SET(peripheral,state) \
        bitWrite(pm, peripheral, state)
